...
> ⚠️ **مستند تاريخي (Historic Baseline — 20 يوليو 2026):** كُتب على **Wails v2 وReact 18** قبل هجرة v3. بنود أُغلقت فعلاً: thresholds التغطية مُفعّلة (`vitest.config.ts:47-52`)، وحصص AI خادمية، وLogout يستدعي الخلفية، وسجلات التطبيق تُدوَّر. المراجع الحية: `docs/features-tracker.md` و`docs/development-plan/TASKS.md`.
