...
### 🟡 [C-04] الحجر الصحي (`MediaTrash`) بلا نافذة استرداد فعليّة — ✅ تم الإصلاح (2026-09-26)
...
**الإصلاح المطبق (2026-09-26):**
1. تم تحديث وقت التعديل فوراً بعد `os.Rename` عبر `os.Chtimes(trashPath, now, now)` في كلا المسارين: التلقائي الدوري (`moveUnreferencedToTrash`) واليدوي الفوري (`CleanUnusedMediaNow`).
2. تصحيح إحصائيات التنظيف اليدوي بحساب الحجم والمقدار فقط بعد نجاح النقل الفعلي إلى الحجر الصحي.
3. تصحيح التوثيق الداخلي في `purgeOldTrash` من «24 ساعة» إلى «30 يوماً».
4. إضافة اختبار انحدار صارم `TestCleanupUnusedMedia_OldUnreferencedFileSurvivesOneCycle` يؤكد بقاء الملفات القديمة جداً (>35 يوماً) في الحجر الصحي وتجديد مهلة الـ 30 يوماً لها.
