...
> ⚠️ **مستند تاريخي (Historic Baseline — 26 يوليو 2026):** كُتب هذا التقرير على **Wails v2 وReact 18**، والمشروع الآن على **Wails v3 (`v3.0.0-beta.23`) وReact 19**. بعض بنوده أُغلقت فعلاً في الكود الحالي (الأسرار أُزيلت، الاختبارات موجودة في 5 حزم Go + 60 ملف واجهة، حدود الذاكرة والتغطية فُعِّلت). **لا تعتمد عليه كحالة راهنة** — المرجع الحي: `docs/features-tracker.md` و`CHANGELOG.md`.
