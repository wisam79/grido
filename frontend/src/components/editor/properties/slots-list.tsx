...
import { CaretDown, Image, GridFour, X } from "@phosphor-icons/react";
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <span className="shrink-0 text-muted-foreground/80 p-1 bg-muted/50 rounded-md">
            {hasImage ? <Image className="w-4 h-4 text-primary/70" weight="regular" /> : <GridFour className="w-4 h-4 opacity-50" weight="regular" />}
          </span>
    ...
    <div className="space-y-1 font-cairo select-none mt-2">
      ...
      >
        <div className="flex items-center gap-2">
          <GridFour className="w-4 h-4 text-muted-foreground" weight="duotone" />
          <h3 className="text-xs font-bold text-foreground">خانات الكولاج</h3>
          <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded-full text-muted-foreground">
            {slots.length}
          </span>
        </div>
        <CaretDown className={cn("w-4 h-4 text-muted-foreground transition-transform duration-200", expanded ? "rotate-180" : "")} weight="bold" />
      </div>
