...
import { SaveImageFromBase64, ApplyMaskToImage } from "../../../../wailsjs/go/main/App";
...
export function ElementProperties({
  ...
  const handleCancelBgRemoval = () => {
    // لا نقوم بإنهاء العامل (Worker) لكي لا نلغي تنزيل النموذج الجاري حالياً، بل نلغي فقط الحوار للمستخدم
    // ليبقى التحميل مستمراً في الخلفية ليكون جاهزاً فوراً في المرة القادمة.
    setIsRemovingBg(false);
    isWorkerBusy = false;
    setBgProgress(0);
    setBgProgressText("");
    toast.info("تم إيقاف عرض المعالجة. التحميل مستمر في الخلفية لسرعة الاستخدام لاحقاً.");
  };
  ...
  const handleRemoveBg = () => {
    ...
    if (!(window as any).globalBgWorker) {
      if (typeof Worker !== "undefined") {
        try {
          (window as any).globalBgWorker = new Worker(
            new URL("../bg-worker.ts", import.meta.url),
            { type: "module" }
          );
        } catch (err) {
          console.error("Failed to initialize background worker:", err);
          toast.error("فشل تهيئة خيط المعالجة المنفصل");
          setIsRemovingBg(false);
          isWorkerBusy = false;
          return;
        }
      } else {
        toast.error("بيئة العمل الحالية لا تدعم Web Workers");
        setIsRemovingBg(false);
        isWorkerBusy = false;
        return;
      }
    }
    const worker = (window as any).globalBgWorker;

    worker.onmessage = (event: MessageEvent) => {
      // إذا ألغى المستخدم العملية، نتجاهل أي رسالة قادمة من العامل
      ...
      if (type === "progress") {
        ...
        (async () => {
          try {
            ...
            // دمج القناع مع الصورة عالية الدقة وحفظها في مجلد الميديا في الخلفية بالكامل
