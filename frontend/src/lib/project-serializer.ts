...
export function domainProjectToProjectFile(dbProj: domain.Project): ProjectFileV1 {
  ...
  const projectFile: ProjectFileV1 = {
    ...
    gridColor: (dbProj as any).gridColor ?? "#000000",
    gridOpacity: (dbProj as any).gridOpacity ?? 0.15,
    gridSubdivisions: (dbProj as any).gridSubdivisions ?? 5,
    gridType: (dbProj as any).gridType ?? "lines",
    snapToGrid: (dbProj as any).snapToGrid ?? false,
    showColumns: (dbProj as any).showColumns ?? false,
    columnsCount: (dbProj as any).columnsCount ?? 12,
    columnsColor: (dbProj as any).columnsColor ?? "rgba(239, 68, 68, 0.08)",
    columnsMargin: (dbProj as any).columnsMargin ?? 20,
    columnsGutter: (dbProj as any).columnsGutter ?? 12,
